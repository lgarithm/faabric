...
TEST_CASE("Test using scatter send functionality (sendMany)", "[transport]")
{
    ...
    std::jthread sendThread([&] {
        ...
        if (msgs.size() == 1) {
            ...
            bufferArray2 = { BYTES(msgs.at(0).data()), BYTES(msgs.at(1).data()) };
            sizeArray2 = { sizeof(int) * msgs.at(0).size(), sizeof(int) * msgs.at(1).size() };
    ...
    if (msgs.size() == 1) {
        ...
        std::vector<std::vector<int>> actual = { std::vector<int>(msgs.at(0).size()), std::vector<int>(msgs.at(1).size()) };
        dst.recvOne(conn, BYTES(actual.at(0).data()), sizeof(int) * actual.at(0).size());
        dst.recvOne(conn, BYTES(actual.at(1).data()), sizeof(int) * actual.at(1).size());
